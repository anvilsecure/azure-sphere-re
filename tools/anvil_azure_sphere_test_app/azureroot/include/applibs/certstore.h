...
///     present or the private key could not be decrypted.</para>
///     <para>ERANGE: <paramref name="certBlobLength"/> or <paramref name="privateKeyBlobLength"/>
///     is zero or greater than CERTSTORE_MAX_CERT_SIZE or the <paramref name="privateKeyPassword"/>
///     length is greater than <see cref="CERTSTORE_MAX_PRIVATE_KEY_PASSWORD_LENGTH"/>.</para>
...
///     A pointer to the password string to decrypt the <paramref name="privateKeyBlob">. The string
///     must be null-terminated and the length must be less than or equals to <see
///     cref="CERTSTORE_MAX_PRIVATE_KEY_PASSWORD_LENGTH"/> bytes (excluding the termination). If
///     <paramref name="privateKeyBlob"/> is encrypted, this field is required.
/// </param>
/// <remarks>
...
///     CERTSTORE_MAX_CERT_SIZE.</para>
///     <para>ENOSPC: there is not enough space in the certificate storage for this
///     certificate.</para>
///     <para>EINVAL: the <paramref name="identifier"/> is not null-terminated or the
///     <paramref name="certBlob"/> has invalid data.</para>
///     <para>EAGAIN: the OS certstore component isn't ready yet.</para>
///     Any other errno may also be specified; such errors aren't deterministic and
///     there's no guarantee that the same behaviour will be retained through system updates.
/// </summary>
...
/// <summary>
///     Renames a certificate with identifier <paramref name="oldIdentifier"/> to have the new
///     identifier <paramref name="newIdentifier"/>. If any other certificate with identifier
///     <paramref name="newIdentifier"/> already exists, it will be replaced. The replacement is
///     atomic. However, any Wifi configuration using the certificates with the old or new
///     identifiers will not see any effect until the configuration is reloaded with <see
///     cref="WifiConfig_ReloadConfig"/>. Any Wifi configuration using the old identifier will
///     need to be updated separately.
///     <para>**Errors** </para>
///     If an error is encountered, returns -1 and sets errno to the error value.
///     <para>EFAULT: <paramref name="oldIdentifier"/> or <paramref name="newIdentifier"/> is
///     NULL.</para>
///     <para>EINVAL: <paramref name="oldIdentifier"/> or <paramref name="newIdentifier"/> is an
///     invalid identifier.</para>
///     <para>ENOENT: the certificate with the identifier <paramref name="oldIdentifier"/> does not
///     exist.</para>
///     <para>EACCES: this operation is not allowed because the CertStore capability
///     is not present.</para>
///     <para>EAGAIN: the OS certstore component isn't ready yet.</para>
...
///     The existing certificate identifier that will be replaced.
/// </param>
/// <param name="newIdentifier">
///     The new certificate identifier that replaces <paramref name ="oldIdentifier"/>.
/// </param>
/// <returns>
///     0 for success, -1 for failure, in which case errno is set to the error value.
/// </returns>
int CertStore_MoveCertificate(const char *oldIdentifier, const char *newIdentifier);
