...
typedef struct WifiConfig_NetworkDiagnostics {
    ...
    /// </summary>
    int32_t error;
    /// <summary>POSIX time when error was recorded</summary>
    time_t timestamp;
    /// <summary>
    /// The certificate error, meaningful only when 'error' indicates AuthenticationFailed.
    /// Below are the error codes which might be assigned to certError:
    ///    Unspecified = 0: Generic error message when certificate validation fails
    ///      Untrusted = 1: Certificate untrusted. Potential reasons:
    ///                     - Unable to retrieve the certificate issuer.
    ///                     - Unable to get CRL or CRL issuer (CRL = Certificate Revocation List).
    ///                     - Self-signed certificate in chain.
    ///                     - Invalid/Unknown CA.
    ///         Revoked = 2: Certificate was revoked.
    ///     NotYetValid = 3: Certificate or CRL not yet valid.
    ///         Expired = 4: Certificate or CRL has expired.
    /// SubjectMismatch = 5: Certificate subject name does not match the name of the entity
    ///                      presenting the certificate.
    /// AltSubjectMismatch = 6: Certificate altsubject name does not match the name of the
    ///                         entity presenting the certificate.
    ///     BadCertificate = 7: Certificate marked as invalid. Potential reasons:
    ///                         - Unable to decrypt certificate or CRL signature.
    ///                         - Unable to decode issuer public key.
    ///                         - Error in Cert Not Before or Cert Not After field.
    ///                         - Error in CRL last update field or CRL next update field.
    /// ServerChainProbeFailed = 8: Server certificate chain validation failed.
    ///   DomainSuffixMismatch = 9: The certificate's domain suffix does not match the domain
    ///                             suffix of the entity presenting the certificate.
    ///         DomainMismatch = 10: The certificate's domain does not match the domain of the
    ///                              entity presenting the certificate.
    ///     InsufficientKeyLen = 11: RSA modulus size is too small to provide adequate security.
    /// </summary>
    int32_t certError;
...
///     characters in length. See <see cref="CertStore_InstallCertificate"/> for details.
/// </param>
/// <returns>
///     0 for success, or -1 for failure, in which case errno is set to the error value.
/// </returns>
